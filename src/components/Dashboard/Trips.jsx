...
function Trips() {
  ...
  const { customPlaces, setcustomPlaces, User_Id, BackendUrl, CurrentTripId, setCurrentTripId, isMobile, setIsMobile } = useContext(RecoveryContext);
  const [showCompleteMessage, setShowCompleteMessage] = useState(false);
  const [showSuccessMessage, setShowSuccessMessage] = useState(false);
  const [showAll, setShowAll] = useState(''); // State to manage show all toggle
  ...
  const handleToggleShowAll = (container) => {
    if (container === showAll) {
      setShowAll('');
    } else {
      setShowAll(container);
    }
  }
    ...
    <div className='trip-div'>
      ...
      <div className="places-div">
        <div className="exploreMore boxes">
          <div className="heading-showallbtn">
            <h2>Explore More Places</h2>
            {isMobile && <button className="show-all-btn" onClick={() => handleToggleShowAll('explore')}>
              {showAll === 'explore' ? "Show Less" : "Show All"}
            </button>}
          </div>
          ...
          <div className="places">
            {filteredPlaces.length > 0 ? (
              filteredPlaces.slice(0, showAll === 'explore' ? filteredPlaces.length : 4).map((place) => (
        ...
        <div className="popularPlaces boxes">
          <div className="heading-showallbtn">
            ...
            {isMobile && <button className="show-all-btn" onClick={() => handleToggleShowAll('topplaces')}>
              {showAll === 'topplaces' ? "Show Less" : "Show All"}
            </button>}
          </div>
          <div className="country-buttons">
          ...
          <div className="places PlacesExplore">
            {filteredPopularPlaces.length > 0 ? (filteredPopularPlaces.slice(0, showAll === 'topplaces' ? filteredPopularPlaces.length : 4).map((place) => (
              <div className="place small-Boxes" key={place.id}>
                <div className="img-div">
                  <img src={place.image} alt={`${place.name} img`} />
                </div>
        ...
        {!Loading ? (customPlaces.length > 0 && <div className="customPlaces boxes">
          <div className="heading-showallbtn">
            <h2>Custom Places</h2>
            {isMobile && <button className="show-all-btn" onClick={() => handleToggleShowAll('customplaces')}>
              {showAll === 'customplaces' ? "Show Less" : "Show All"}
            </button>}
          </div>
          <div className="country-buttons">
          ...
          <div className="places CustomExplore">
            {filteredCustomPlaces.length > 0 ? (filteredCustomPlaces.slice(0, showAll === 'customplaces' ? filteredCustomPlaces.length : 4).map((place) => (
              <div className="place small-Boxes" key={place.id}>
                <div className="img-div">
                  <img src={place.image} alt={`${place.name} img`} />
                </div>
